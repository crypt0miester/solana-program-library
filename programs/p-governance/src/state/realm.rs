...
use {
    ...
    }, p_spl_governance_addin_api::voter_weight::VoterWeightAction, p_spl_governance_tools::account::{
        assert_is_valid_account_of_types, get_account_type, to_bytes_with_len, AccountMaxSize, DataLen, IsInitialized
    }, pinocchio::{
        account_info::{AccountInfo, Ref}, memory::sol_memcpy, program_error::ProgramError, pubkey::{find_program_address, Pubkey},
    }, pinocchio_utils_macro::{FromEnum, FromU8, IsEnum, PInstruction, PStruct}, std::slice::Iter
};
...
impl DataLen for RealmConfigArgs {
    const LEN: usize = core::mem::size_of::<RealmConfigArgs>();
}
...
impl DataLen for GoverningTokenConfigArgs {
    const LEN: usize = core::mem::size_of::<GoverningTokenConfigArgs>();
}
...
impl DataLen for GoverningTokenConfigAccountArgs {
    const LEN: usize = core::mem::size_of::<GoverningTokenConfigAccountArgs>();
}

/// SetRealmAuthority instruction action
#[derive(Clone, Debug, PartialEq, Eq, FromEnum, FromU8)]
#[derive(PInstruction)]
#[derive(IsEnum)]
...
impl DataLen for SetRealmAuthorityAction {
    const LEN: usize = core::mem::size_of::<SetRealmAuthorityAction>();
}
...
impl RealmV2 {
    ...
    pub fn serialize(self, data: &mut [u8]) -> Result<(), ProgramError> {
        ...
        if self.account_type == GovernanceAccountType::RealmV2 {
            ...
            unsafe {
                sol_memcpy(data, to_bytes_with_len::<RealmV1>(&realm_data_v1, data_len), data_len);
            }
        }
    ...
    pub fn from_account_info<'a>(account_info: &'a AccountInfo, program_id: &'a Pubkey) -> Result<Ref<'a, Self>, ProgramError> {
        ...
        if data[0] != GovernanceAccountType::RealmV2 as u8 {
    ...
    /// Return a `RealmV2` from the given bytes.
    ///
    ...
    /// # Safety
    ///
    /// The caller must ensure that `bytes` contains a valid representation of `RealmV2`.
    #[inline(always)]
    pub unsafe fn from_bytes_unchecked(bytes: &[u8]) -> &Self {
        &*(bytes.as_ptr() as *const Self)
    }

    /// Return a mutable `RealmV2` from the given bytes.
    ///
    /// This method validates that `bytes` has at least the minimum required
    /// length.
    #[inline(always)]
...
pub fn get_realm_data(
    ...
    if account_type == GovernanceAccountType::RealmV1 {
        let realm_data_v1 = RealmV1::from_account_info( realm_info, program_id)?;

        return Ok(RealmV2 {
            account_type,
            community_mint: realm_data_v1.community_mint,
            config: realm_data_v1.config.clone(),
            reserved: realm_data_v1.reserved,
            legacy1: 0,
            authority: realm_data_v1.authority,
            name: realm_data_v1.name.clone(),
            // Add the extra reserved_v2 padding
            reserved_v2: [0; 128],
        });
    }

    let realm_data = RealmV2::from_account_info( realm_info, program_id)?;
    Ok(realm_data.clone())
}
...
pub fn get_realm_address(program_id: &Pubkey, name: &str) -> Pubkey {
    find_program_address(&get_realm_address_seeds(name), program_id).0
}
