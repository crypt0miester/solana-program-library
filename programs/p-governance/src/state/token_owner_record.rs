...
use {
    ...
    p_spl_governance_addin_api::voter_weight::VoterWeightAction,
    p_spl_governance_tools::account::{
        get_account_data, get_account_type, to_bytes, to_bytes_with_len, AccountMaxSize, DataLen, IsInitialized
    },
    pinocchio::{
        account_info::AccountInfo, memory::sol_memcpy, program_error::ProgramError, pubkey::{find_program_address, Pubkey}
    },
    std::slice::Iter,
};
...
impl DataLen for TokenOwnerRecordV2 {
    const LEN: usize = core::mem::size_of::<TokenOwnerRecordV2>();
}
...
impl TokenOwnerRecordV2 {
    ...
    pub fn serialize(self, data: &mut [u8]) -> Result<(), ProgramError> {
        if self.account_type == GovernanceAccountType::TokenOwnerRecordV2 {
            ...
            unsafe {
                sol_memcpy(data, to_bytes_with_len::<TokenOwnerRecordV1>(&token_owner_record_data_v1, self.get_max_size().unwrap()), self.get_max_size().unwrap());
            }
        }
...
pub fn get_token_owner_record_data_for_seeds(
    ...
    let (token_owner_record_address, _) =
        find_program_address(token_owner_record_seeds, program_id);
