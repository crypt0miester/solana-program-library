...
use {
    crate::state::{
        ...
        realm::RealmConfig,
    }, p_spl_governance_tools::account::{DataLen, IsInitialized}, pinocchio::{account_info::{AccountInfo, Ref}, program_error::ProgramError, pubkey::Pubkey, sysvars::clock::{Slot, UnixTimestamp}}
};
...
impl RealmV1 {
    ...
    pub fn from_account_info<'a>(
        ...
        if data[0] != GovernanceAccountType::RealmV1 as u8 {
    ...
    /// Return a `RealmV1` from the given bytes.
    ///
    ...
    /// # Safety
    ///
    /// The caller must ensure that `bytes` contains a valid representation of `RealmV1`.
    #[inline(always)]
    pub unsafe fn from_bytes_unchecked(bytes: &[u8]) -> &Self {
        &*(bytes.as_ptr() as *const Self)
    }

    /// Return a mutable `RealmV1` from the given bytes.
    ///
    /// This method validates that `bytes` has at least the minimum required
    /// length.
    #[inline(always)]
...
impl DataLen for TokenOwnerRecordV1 {
    const LEN: usize = core::mem::size_of::<TokenOwnerRecordV1>();
}
...
impl DataLen for GovernanceV1 {
    const LEN: usize = core::mem::size_of::<GovernanceV1>();
}
...
impl DataLen for ProposalV1 {
    const LEN: usize = core::mem::size_of::<ProposalV1>();
}
...
impl ProposalV1 {
    ...
    /// Returns a `ProposalV1` from account info.
    ///
    /// This method will perform the following validations on the account info:
    ///  1. Owner check: it must match the program.
    ///  2. Account discriminator: it must match [`GovernanceAccountType::ProposalV1`].
    ///  3. Borrow data: it must be allowed to borrow the account data.
    #[inline]
    pub fn from_account_info<'a>(
        account_info: &'a AccountInfo,
        ...
        let data = account_info.try_borrow_data()?;
        if data[0] != GovernanceAccountType::ProposalV1 as u8 {
            return Err(ProgramError::InvalidAccountData);
        }
        // SAFETY: `data` was validated to have the correct owner and discriminator.
        Ok(Ref::map(data, |data| unsafe {
    ...
    /// Return a `ProposalV1` from the given bytes.
    ///
    /// This method validates that `bytes` has at least the minimum required
    /// length.
    #[inline(always)]
    pub fn from_bytes(bytes: &[u8]) -> Result<&Self, ProgramError> {
        // SAFETY: `bytes` was validated to have the expected length
        // to hold a `ProposalV1` reference.
        Ok(unsafe { Self::from_bytes_unchecked(bytes) })
    }
    ...
    /// # Safety
    ///
    /// The caller must ensure that `bytes` contains a valid representation of `ProposalV1`.
    #[inline(always)]
    pub unsafe fn from_bytes_unchecked(bytes: &[u8]) -> &Self {
        &*(bytes.as_ptr() as *const Self)
    }

    /// Return a mutable `ProposalV1` from the given bytes.
    ///
    /// This method validates that `bytes` has at least the minimum required
    /// length.
    #[inline(always)]
...
impl DataLen for SignatoryRecordV1 {
    const LEN: usize = core::mem::size_of::<SignatoryRecordV1>();
}
...
impl DataLen for ProposalInstructionV1 {
    const LEN: usize = core::mem::size_of::<ProposalInstructionV1>();
}
...
impl DataLen for VoteRecordV1 {
    const LEN: usize = core::mem::size_of::<VoteRecordV1>();
}