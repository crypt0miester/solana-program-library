...
use {
    ...
    }, p_spl_governance_tools::{account::{assert_is_valid_account_of_types, extend_account_size, get_account_data, get_account_type, to_bytes, AccountMaxSize, DataLen, IsInitialized}, error::GovernanceToolsError}, pinocchio::{account_info::AccountInfo, instruction::Seed, memory::sol_memcpy, program_error::ProgramError, pubkey::{find_program_address, Pubkey}, seeds, sysvars::rent::Rent, ProgramResult}, pinocchio_utils_macro::PStruct
};

/// Governance config
#[derive(Clone, Debug, PartialEq, Eq, Copy, PStruct)]
...
impl DataLen for GovernanceConfig {
    const LEN: usize = core::mem::size_of::<GovernanceConfig>();
}
...
impl DataLen for GovernanceV2 {
    const LEN: usize = core::mem::size_of::<GovernanceV2>();
}
...
impl GovernanceV2 {
    ...
    pub fn get_governance_address_p_seeds(&self) -> Result<[Seed; 3], ProgramError> {
        let seeds = match self.account_type {
            GovernanceAccountType::GovernanceV1 | GovernanceAccountType::GovernanceV2 => {
                get_governance_address_p_seeds(&self.realm, &self.governed_account)
            }
            GovernanceAccountType::ProgramGovernanceV1
            | GovernanceAccountType::ProgramGovernanceV2 => {
                get_program_governance_address_p_seeds(&self.realm, &self.governed_account)
            }
            GovernanceAccountType::MintGovernanceV1 | GovernanceAccountType::MintGovernanceV2 => {
                get_mint_governance_address_p_seeds(&self.realm, &self.governed_account)
            }
            GovernanceAccountType::TokenGovernanceV1 | GovernanceAccountType::TokenGovernanceV2 => {
                get_token_governance_address_p_seeds(&self.realm, &self.governed_account)
            }
            GovernanceAccountType::Uninitialized
            | GovernanceAccountType::RealmV1
            | GovernanceAccountType::TokenOwnerRecordV1
    ...
    pub fn serialize(self, data: &mut [u8]) -> ProgramResult {
        if is_governance_v2_account_type(&self.account_type) {
            ...
            unsafe {
                sol_memcpy(data, to_bytes::<GovernanceV1>(&governance_data_v1), self.get_max_size().unwrap());
            }
        }
    ...
    /// and TokenGovernance)
    pub unsafe fn serialize_as_governance_v2(
        mut self,
        ...
        if let Some(governance_v2_type) = try_get_governance_v2_type_for_v1(&self.account_type) {
            ...
            self.serialize(&mut governance_info.borrow_mut_data_unchecked()[..]) 
...
pub fn get_program_governance_address_p_seeds<'a>(
    realm: &'a Pubkey,
...
pub fn get_mint_governance_address_p_seeds<'a>(
    realm: &'a Pubkey,
    governed_mint: &'a Pubkey,
) -> [Seed<'a>; 3] {
    // 'program-governance' prefix ensures uniqueness of the PDA
...
pub fn get_token_governance_address_p_seeds<'a>(
    realm: &'a Pubkey,
    governed_token: &'a Pubkey,
) -> [Seed<'a>; 3] {
    // 'token-governance' prefix ensures uniqueness of the PDA
    ...
    seeds!(b"token-governance", realm.as_ref(), governed_token.as_ref()   )
}
...
/// Returns Governance PDA seeds
pub fn get_governance_address_p_seeds<'a>(
    realm: &'a Pubkey,
    governed_account: &'a Pubkey,
) -> [Seed<'a>; 3] {
    seeds!(
