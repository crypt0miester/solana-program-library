...
impl DataLen for SignatoryRecordV2 {
    const LEN: usize = core::mem::size_of::<SignatoryRecordV2>();
}
...
impl SignatoryRecordV2 {
    ...
    pub fn serialize(self, data: &mut [u8]) -> Result<(), ProgramError> {
        if self.account_type == GovernanceAccountType::SignatoryRecordV2 {
            ...
            unsafe {
                sol_memcpy(data, to_bytes_with_len::<SignatoryRecordV1>(&signatory_record_data_v1, Self::LEN), Self::LEN);
            }
        }
...
pub fn get_signatory_record_data_for_seeds(
    ...
    let (signatory_record_address, _) = find_program_address(
        &get_signatory_record_address_seeds(proposal, signatory),
        program_id,
    );
