...
use {
    crate::{
        ...
        tools::{next_account_info, structs::Reserved110},
    },
    p_spl_governance_tools::account::{
        get_account_data, AccountMaxSize, DataLen, IsInitialized
    },
    pinocchio::{account_info::AccountInfo, program_error::ProgramError, pubkey::{find_program_address, Pubkey}},
    std::slice::Iter,
};
...
impl DataLen for RealmConfigAccount {
    const LEN: usize = core::mem::size_of::<RealmConfigAccount>();
}
...
pub fn get_realm_config_address(program_id: &Pubkey, realm: &Pubkey) -> Pubkey {
    find_program_address(&get_realm_config_address_seeds(realm), program_id).0
}
