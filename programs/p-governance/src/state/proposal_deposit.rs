...
use {
    ...
    p_spl_governance_tools::account::{get_account_data, AccountMaxSize, DataLen, IsInitialized},
};
...
impl DataLen for ProposalDeposit {
    const LEN: usize = core::mem::size_of::<ProposalDeposit>();
}
...
mod test {
    ...
    #[test]
    fn test_max_size() {
        let new_pubkey: Pubkey = [0; 32].into();
        // Arrange
        let proposal_deposit_data = ProposalDeposit {
            account_type: GovernanceAccountType::ProposalDeposit,
            proposal: new_pubkey,
            deposit_payer: new_pubkey,
            reserved: [0; 64],
        };

        // Act
        let size = unsafe { to_bytes(&proposal_deposit_data) }.len();
