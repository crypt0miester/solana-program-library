...
pub fn process_set_realm_config(
    ...
    if realm_config_info.data_is_empty() {
        ...
        let realm_config_data_mut = unsafe { realm_config_info.borrow_mut_data_unchecked() };
        let data_len = realm_config_info.data_len();
        unsafe {
            sol_memcpy(
                realm_config_data_mut,
                to_bytes_with_len(&realm_config_data, data_len),
                data_len,
            );
        }
    }
    ...
    let realm_info_data = unsafe { realm_info.borrow_mut_data_unchecked() };
    realm_data.serialize(realm_info_data)?;
