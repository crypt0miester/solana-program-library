...
use {
    crate::{
        ...
        tools::{
            ephermal_signers::EPHERMAL_SIGNER_SEED, next_account_info,
            transaction_message::TransactionMessage,
        },
    }, p_spl_governance_tools::account::create_and_serialize_account_signed, pinocchio::{account_info::AccountInfo, pubkey::{find_program_address, Pubkey}, sysvars::rent::Rent, ProgramResult}, std::cmp::Ordering
};
...
pub fn process_create_versioned_transaction_account(
    program_id: &Pubkey,
    option_index: u8,
    ephemeral_signers: u8,
    transaction_index: u16,
    ...
    proposal_versioned_transaction_info: &AccountInfo,
    ...
    let ephemeral_signer_bumps: Vec<u8> = (0..ephemeral_signers)
        .map(|ephemeral_signer_index| {
            ...
            let (_, bump) = find_program_address(ephemeral_signer_seeds, program_id);
            bump
        })
        .collect();
