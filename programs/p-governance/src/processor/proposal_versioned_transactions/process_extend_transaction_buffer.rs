...
pub fn process_extend_transaction_buffer(
    ...
    log!("Buffer size: {} -> {}", new_data_size, current_buffer_size);
    ...
    let proposal_transaction_buffer_info_data =
        unsafe { proposal_transaction_buffer_info.borrow_mut_data_unchecked() };
    // Serialize the modified transaction buffer back to account data
    proposal_transaction_buffer_data
        .serialize(proposal_transaction_buffer_info_data)?;
