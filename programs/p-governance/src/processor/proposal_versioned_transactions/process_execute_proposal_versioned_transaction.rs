...
use {
    crate::{
        ...
        tools::{
            ...
            executable_transaction_message::ExecutableTransactionMessage, next_account_info,
        },
    },
    pinocchio::{
...
pub fn process_execute_versioned_transaction(
    ...
    let (governance_pubkey, bump_seed) =
        find_program_address(&governance_seeds, program_id);
    let bump = &[bump_seed];
    ...
    let proposal_versioned_transaction_info_data =
        unsafe { proposal_versioned_transaction_info.borrow_mut_data_unchecked() };
    ...
    proposal_versioned_transaction_data.serialize(proposal_versioned_transaction_info_data)?;
